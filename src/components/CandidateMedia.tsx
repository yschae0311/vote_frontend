...
export function CandidateMedia({
  ...
  const src = thumbnailImageUrl(cand.image_url);
  ...
  if (src) {
    return (
      <img
        ...
        className={className}
        style={{
          aspectRatio: auto ? undefined : ratio,
          borderRadius: round,
          width: '100%',
          height: auto ? '100%' : undefined,
          objectFit: fit,
        }}
      />
    );
  ...
  return (
    <div
      ...
      style={{ '--ph-h': cand.tint, aspectRatio: auto ? undefined : ratio, borderRadius: round, height: auto ? '100%' : undefined } as React.CSSProperties}
    >
      <span className="ph-emoji" style={{ fontSize: emojiSize }}>{EMOJI[cand.name] || '✨'}</span>
      <span className="ph-label" style={{ position: 'absolute', bottom: 8, right: 8 }}>후보 이미지</span>
    </div>
  );
}
